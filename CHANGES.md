...
* Fixed: A copy of Mai Analytics bundled inside another plugin could take over from an active standalone install, leaving the site running older code with no indication. Whichever copy loaded first used to win; a bundled copy now always steps aside for the standalone plugin.
...
* Fixed: A provider sync error was shown against whichever provider you were looking at, so a Matomo failure could appear under Google Analytics. Errors now appear only on the provider actually doing the syncing.
* Fixed: The Analytics item in the admin menu did not highlight while you were on its page.
* Changed: Declared a minimum PHP version of 8.1.
...
* Fixed: Google Analytics (via Site Kit) view syncing failed on every cron run, reporting "Site Kit can't access the relevant data from Analytics because you haven't granted all permissions requested during setup" even on correctly connected sites. Site Kit binds its Google credentials to whichever user is current early in the WordPress load, and under cron that is nobody — so switching users at request time, as this plugin did, never reached it. The GA4 client is now built bound to the Site Kit module owner directly, the way Site Kit's own background jobs work, so syncing no longer depends on who is logged in. Affected sites saw web view counts refresh only when the connected Google account's user triggered a sync from wp-admin; counts were preserved, not lost, and resume updating on the next sync.
...
* Changed: [Developers] A Site Kit report response in an unrecognized shape is now reported as a sync error instead of being read as "no views". Previously an upstream contract change would have stopped syncing indefinitely while the admin still showed a healthy provider and no error was logged anywhere.
* Changed: [Developers] Provider sync errors now name the window and date range that failed, and a failed window stops the remaining requests in that batch rather than spending further API calls on a result already committed to failure.
